   ...
   if (ht->isleaf()){
	 ...
	 ht->count()--;
      ...
      for (ni = neweql->begin(); ni != neweql->end(); ni++){
		 // for(int i=0; i<(*ni)->prefix().size();i++){
		//	  cout<<(*ni)->prefix()[i] << " ";
		 // }

         ht->add_element(*ni);
	 ht->count()++;
	 //cout << ", size nodelist: " << (*ni)->nlist().size() << endl;
...
void get_Fk()
{
  ...
  for (int iter=3; !CandK->isempty(); iter++){
	...
	//if(iter>3)
	//	cout << "Number of false negative - " << num_false_neg << endl;
...
int main(int argc, char **argv)
{
   ...
   cout << "preprocessing time = " << tottime - kernel_time << endl; 
    
    //Elaheh
    //cout << "Pruning time: " << prune_te << "\n";
   exit(0);
}
